...
2) Install an SD card with the latest version of Raspbian
     ...
     - Expand the filesystem with `sudo raspi-config`
     - Set the correct time zone `sudo raspi-config`
     - Update the sources list `sudo apt-get update`
     - Upgrade your system `sudo apt-get -y dist-upgrade`
     - Reboot `sudo reboot`
